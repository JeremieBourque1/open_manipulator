...
namespace to_twist
{
  class gamepadToTwist
  {
  public:
    gamepadToTwist() : spinner_(1)
    {
      joy_sub_ = n_.subscribe("joy", 1, &gamepadToTwist::joyCallback, this);
      twist_pub_ = n_.advertise<geometry_msgs::TwistStamped>("/servo_server/delta_twist_cmds", 1);
      joint_delta_pub_ = n_.advertise<control_msgs::JointJog>("/servo_server/delta_joint_cmds", 1);
...
{
  ros::init(argc, argv, "gamepad_to_twist");

  to_twist::gamepadToTwist to_twist;
